...
df.rename(
    # index=str, 
    columns = {'Life expectancy at birth (years)' : 'LEABY'} ,
    inplace=True 
)
...
df.rename(index=str, columns={'Life expectancy at birth (years)' : 'LEABY'} )
df.head()
...
The two bar charts look similar, though on the GDP chart has a wide range of averages which causes scale problems for comparision. 
...
plt.savefig("1_Codecademy_violin_plot_Life_Expectancy.png")
plt.show()
...
# Zimbabwe saw the greatest change in life expactancy between 2003 adn 2015. 
# The US, Mexico, and Germany have all remained relatively flat, with only modest growth over the 15 year period. 
# The appears to be some corelation between GDP and life expectancy, though the exact relationship between the two remains unclear from these charts. 
# Zimbabwe was invovled in two conflict which likely affected the population's life expectancy. The end of the Kivu Conflict in 2009 correlates to a an rise in life expenacy and GDP. 
...
g.fig.suptitle('Scatter Plots of GDP and Life Expectancy Data (2000-2015)')

plt.savefig("3_codecademy_Scatter Plots of GDP and Life Expectancy Data.png")
...
# China moves along the X axis the most over the years
# Zimbabwe moves the most long the Y axis over the years
# Not especially given both countries recent history
# It depends on what information I'm trying to see. The outliers and movement became clearer with these charts. 
...
plt.subplots_adjust(top=0.9)
g3.fig.suptitle('Line Plots for Life Expectancy (2000-2015)')

plt.savefig("4_codecademy_Line Plots for Life Expectancy.png")
plt.show()
...
#add title
plt.subplots_adjust(top=0.9)
g3.fig.suptitle('Line Plots for GDP (2000-2015)')

#save File
plt.savefig("5_codecademy_Line Plots for GDP.png")
#display chart
plt.show()
...
# The U.S. has the highest GDP, while Zimbabwe has the lowest
...
# Germany has the highest life expectancy, while Zimbabewe has the lowest
...
# China underwent major economic restructuting, moving away from socialism to an oligarchy.
